...
export const getAllChatRooms = () => async (dispatch) => {
  try {
    ...
    await axios
      .get(`${url}/get-chatRooms`, { withCredentials: true })
      .then((response) => {
...
export const getChats = (chatId) => async (dispatch) => {
  try {
    ...
    await axios
      .get(`${url}/get-chats/${chatId}`, { withCredentials: true })
      .then((response) => {
...
export const createRoom =
  (prompt, navigate, results, setResults, userId) => async (dispatch) => {
    try {
...
export const askQuestion =
  (prompt, chatId, setResults, userId) => async (dispatch) => {
    try {
      dispatch({ type: ASK_QUESTION_REQUEST });
      ...
      await axios
        ...
        .then((response) => {
          ...
          setResults((p) => [
            ...p,
            { role: "model", parts: response.data.text },
          ]);
        });
    } catch (err) {
      dispatch({
...
export const deleteChatRoom = (chatId) => async (dispatch) => {
  try {
    ...
    await axios
      .delete(`${url}/delete-chatRoom/${chatId}`, { withCredentials: true })
      .then((response) => {
        dispatch({
